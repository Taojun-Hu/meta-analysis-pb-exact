...
data.generate <- lapply(
    1:n.datasets, FUN = function(ind){
        ...
        t1 <- runif( s, min = 250, max = 350 )
        t0 <- runif( s, min = 250, max = 350 )
        y_all <- runif( s, min = 5, max = 20) %>% round()
        study.pi <- plogis( log(t1/t0) + study.theta )
        
        y1 = rbinom( s, size = y_all, prob = study.pi )
        y0 = y_all - y1
        
        data.tmp <- data.frame( y1 = y1, y0 = y0, t1 = t1, t0 = t0 ) %>% 
            dplyr::mutate( cx = (
                (y1 ==0)|(y0 == 0)
            )*0.5 ) %>% 
        ...
        expectation <- (sqrt(y_all)*t.theta/(exp(-(t.theta + log(t1/t0))/2) + exp((t.theta + log(t1/t0))/2)) -1/(8*sqrt(y_all))*(
            t.theta*(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))
        ) + 1/2*t.sigma**2*(
            sqrt(y_all)*(
                exp(-3*(t.theta + log(t1/t0))/2)*(1 + t.theta/4) - exp(3*(t.theta + log(t1/t0))/2)*(1 - t.theta/4) + 
                    exp(-(t.theta + log(t1/t0))/2)*(1 - 5*t.theta/4) - exp((t.theta + log(t1/t0))/2)*(1 + 5*t.theta/4)
            )/(
                exp(-(t.theta + log(t1/t0))/2) + exp((t.theta + log(t1/t0))/2)
            )**4 - 1/(8*sqrt(y_all))*(
                exp((t.theta + log(t1/t0))/2)*(1+t.theta/4) + exp(-(t.theta + log(t1/t0))/2)*(t.theta/4 - 1)
            )
        ) + 1/24*3*t.sigma**4*(
            4*sqrt(y_all)*(
                -6*(1/2*exp((t.theta + log(t1/t0))/2) -1/2*exp(-(t.theta + log(t1/t0))/2) )**3/( exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2) )**4 + 
                    6*(1/4*exp(-(t.theta + log(t1/t0))/2) + 1/4*exp((t.theta + log(t1/t0))/2))*(1/2*exp((t.theta + log(t1/t0))/2) - 1/2*exp(-(t.theta + log(t1/t0))/2))/(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))**3 - 
                    (1/8*exp((t.theta + log(t1/t0))/2) - 1/8*exp(-(t.theta + log(t1/t0))/2))/(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))**2
            ) + sqrt(y_all)*t.theta*(
                24*(1/2*exp((t.theta + log(t1/t0))/2) - 1/2*exp(-(t.theta + log(t1/t0))/2))**4/(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))**5 - 
                    36*(1/4*exp(-(t.theta + log(t1/t0))/2) + 1/4*exp((t.theta + log(t1/t0))/2))*(1/2*exp((t.theta + log(t1/t0))/2) - 1/2*exp(-(t.theta + log(t1/t0))/2))**2/(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))**4 +
                    8*(1/8*exp((t.theta + log(t1/t0))/2) - 1/8*exp(-(t.theta + log(t1/t0))/2))*(1/2*exp((t.theta + log(t1/t0))/2) - 1/2*exp(-(t.theta + log(t1/t0))/2))/(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))**3 - 
                    (1/16*exp((t.theta + log(t1/t0))/2) +1/16*exp(-(t.theta + log(t1/t0))/2))/(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))**2 + 
                    6*(1/4*exp(-(t.theta + log(t1/t0))/2) + 1/4*exp((t.theta + log(t1/t0))/2))**2/(exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2))**3
            ) - 1/(8*sqrt(y_all))*(exp((t.theta + log(t1/t0))/2)*(1/2 + (t.theta + log(t1/t0))/16) + exp(-(t.theta + log(t1/t0))/2)*((t.theta + log(t1/t0))/16 - 1/2) )
        ) )
        
        variance <- ((
            sqrt(y_all)*(
                sqrt(t1/t0)*exp(t.theta/2)*(1 - t.theta/2) + sqrt(t0/t1)*exp(-t.theta/2)*(1 + t.theta/2)
            )/( exp((t.theta + log(t1/t0))/2) + exp(-(t.theta + log(t1/t0))/2) )**2 - 
                1/(8*sqrt(y_all))*(
                    sqrt(t1/t0)*exp(t.theta/2)*(1 + t.theta/2) + sqrt(t0/t1)*exp(-t.theta/2)*(1 - t.theta/2)
                )
        )**2*t.sigma**2 + (
            ...
            ((1/2 - 1/(1 + exp(theta*t.sigma + t.theta + log(t1/t0))))*(
...
op.org.all <- sapply( data.org.all, FUN = function(x){ c(sum(x[['y1']] + x[['y0']] ), 
                                                         sum(x[['t1']] + x[['t0']] ))  }  ) %>% rowSums() %>% `names<-`(NULL)
print( op.org.all[1]/op.org.all[2])
print('Average events proportion (after select)')
op.select.all <- sapply( data.select.all, FUN = function(x){c(sum(x[['y1']] + x[['y0']] ), 
                                                              sum(x[['t1']] + x[['t0']] ))  }  ) %>% rowSums() %>% `names<-`(NULL)
...
estimate.fun.all <- function(data = data.org, start.ini = c(-3.6, 0.28)){
    ...
    llk <- function(par){
        ...
        prob.prior <- cubature::hcubature( f = function(theta){
            dbinom( y1, y_all, prob = plogis( sigma.ini*theta + theta.ini + log(t1/t0) ) )*dnorm(theta)
...
estimate.fun.asymp <- function(data = data.select, start.ini = c(-3.6, 0.28, 0.4), method = 'nlm',  p = 0.7){
    ...
    llk <- function( par ){
        ...
        expectation <- (sqrt(y_all)*theta.ini/(exp(-(theta.ini + log(t1/t0))/2) + exp((theta.ini + log(t1/t0))/2)) -1/(8*sqrt(y_all))*(
            theta.ini*(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))
        ) + 1/2*sigma.ini**2*(
            sqrt(y_all)*(
                exp(-3*(theta.ini + log(t1/t0))/2)*(1 + theta.ini/4) - exp(3*(theta.ini + log(t1/t0))/2)*(1 - theta.ini/4) + 
                    exp(-(theta.ini + log(t1/t0))/2)*(1 - 5*theta.ini/4) - exp((theta.ini + log(t1/t0))/2)*(1 + 5*theta.ini/4)
            )/(
                exp(-(theta.ini + log(t1/t0))/2) + exp((theta.ini + log(t1/t0))/2)
            )**4 - 1/(8*sqrt(y_all))*(
                exp((theta.ini + log(t1/t0))/2)*(1+theta.ini/4) + exp(-(theta.ini + log(t1/t0))/2)*(theta.ini/4 - 1)
            )
        ) + 1/24*3*sigma.ini**4*(
            4*sqrt(y_all)*(
                -6*(1/2*exp((theta.ini + log(t1/t0))/2) -1/2*exp(-(theta.ini + log(t1/t0))/2) )**3/( exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2) )**4 + 
                    6*(1/4*exp(-(theta.ini + log(t1/t0))/2) + 1/4*exp((theta.ini + log(t1/t0))/2))*(1/2*exp((theta.ini + log(t1/t0))/2) - 1/2*exp(-(theta.ini + log(t1/t0))/2))/(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))**3 - 
                    (1/8*exp((theta.ini + log(t1/t0))/2) - 1/8*exp(-(theta.ini + log(t1/t0))/2))/(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))**2
            ) + sqrt(y_all)*theta.ini*(
                24*(1/2*exp((theta.ini + log(t1/t0))/2) - 1/2*exp(-(theta.ini + log(t1/t0))/2))**4/(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))**5 - 
                    36*(1/4*exp(-(theta.ini + log(t1/t0))/2) + 1/4*exp((theta.ini + log(t1/t0))/2))*(1/2*exp((theta.ini + log(t1/t0))/2) - 1/2*exp(-(theta.ini + log(t1/t0))/2))**2/(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))**4 +
                    8*(1/8*exp((theta.ini + log(t1/t0))/2) - 1/8*exp(-(theta.ini + log(t1/t0))/2))*(1/2*exp((theta.ini + log(t1/t0))/2) - 1/2*exp(-(theta.ini + log(t1/t0))/2))/(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))**3 - 
                    (1/16*exp((theta.ini + log(t1/t0))/2) +1/16*exp(-(theta.ini + log(t1/t0))/2))/(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))**2 + 
                    6*(1/4*exp(-(theta.ini + log(t1/t0))/2) + 1/4*exp((theta.ini + log(t1/t0))/2))**2/(exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2))**3
            ) - 1/(8*sqrt(y_all))*(exp((theta.ini + log(t1/t0))/2)*(1/2 + (theta.ini + log(t1/t0))/16) + exp(-(theta.ini + log(t1/t0))/2)*((theta.ini + log(t1/t0))/16 - 1/2) )
        ) )
        
        variance <- ((
            sqrt(y_all)*(
                sqrt(t1/t0)*exp(theta.ini/2)*(1 - theta.ini/2) + sqrt(t0/t1)*exp(-theta.ini/2)*(1 + theta.ini/2)
            )/( exp((theta.ini + log(t1/t0))/2) + exp(-(theta.ini + log(t1/t0))/2) )**2 - 
                1/(8*sqrt(y_all))*(
                    sqrt(t1/t0)*exp(theta.ini/2)*(1 + theta.ini/2) + sqrt(t0/t1)*exp(-theta.ini/2)*(1 - theta.ini/2)
                )
        )**2*sigma.ini**2 + (
            ...
            ((1/2 - 1/(1 + exp(theta*sigma.ini + theta.ini + log(t1/t0))))*(
        ...
        pselect <- function(alpha, print = FALSE){
            ...
            select.tmp <- pnorm(
                ( beta.ini*expectation + alpha )/sqrt(
                    1 + beta.ini**2*(
                        1 + variance)
                ) )
        ...
        prob.prior <- cubature::hcubature( f = function(theta){
            dbinom( y1, y_all, prob = plogis( sigma.ini*theta + theta.ini + log(t1/t0)  ) )*dnorm(theta)
        }, lowerLimit = -5, upperLimit = 5, fDim = nstudy, tol = 1e-6 )[['integral']]
        
        prob.denom <- pnorm(
            ( beta.ini*expectation+ alpha.opt )/sqrt(
                1 + beta.ini**2*(
                    1 + variance)
            ) )
